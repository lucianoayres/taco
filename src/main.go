...
// parseArguments handles the command-line arguments and returns the output filename, directories to process, and included extensions.
    ...
    if *includeExt != "" {
        ...
        for _, ext := range splitExt {
            ...
            if trimmedExt != "" {
                ...
                extensions = append(extensions, strings.ToLower(trimmedExt))
            }
        }
    }

    return *outputFileName, directories, extensions
}
...
func concatenateFiles(outputFilePath string, directories []string, excludedPaths map[string]struct{}, includeExts []string) error {
    ...
    for _, dir := range directories {
        filesProcessed, err := processDirectory(dir, &outputFile, outputFilePath, excludedPaths, includeExts)
        if err != nil {
...
func processDirectory(dir string, outputFile **os.File, outputFilePath string, excludedPaths map[string]struct{}, includeExts []string) (bool, error) {
    ...
    for _, entry := range entries {
        ...
        if entry.IsDir() {
            ...
            subdirProcessed, err := processDirectory(path, outputFile, outputFilePath, excludedPaths, includeExts)
            if err != nil {
                return false, err
            }
            if !subdirProcessed {
            ...
            if shouldIncludeFile(path, includeExts) && isTextFile(path) {
...
// shouldIncludeFile determines if a file should be included based on the provided extensions.
// If includeExts is empty, it includes all text files.
func shouldIncludeFile(filePath string, includeExts []string) bool {
    if len(includeExts) == 0 {
        return true // No extension filter applied
    }
...
func run() error {
    ...
    if err := concatenateFiles(outputFilePath, directories, excludedPaths, includeExts); err != nil {
        return err
    }
    return nil
}
